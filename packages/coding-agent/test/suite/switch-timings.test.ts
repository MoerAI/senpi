 ...
 * switch path must be imported AFTER the env stub. resetModules + dynamic
 * import keeps the faux provider and the session on one module graph.
 */
describe("session switch phase timings", () => {
	...
	it("records a switch timing namespace covering every runtime-owned switch phase", async () => {
		...
		const tempDir = join(tmpdir(), `pi-switch-timings-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		const target = SessionManager.create(tempDir, sessionDir);
		...
		const result = await runtime.switchSession(targetPath as string);

		expect(result).toEqual({ cancelled: false });
		const labels = getTimings("switch").map((entry) => entry.label);
		expect(labels).toEqual(["beforeSwitch", "open", "abort", "shutdown", "dispose", "apply", "rebind"]);
	});
});
