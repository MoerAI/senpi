...
- `lease-file.ts`: a lease is never visible half-written: it is written to a private temp file and published with `link` (exclusive) or `rename` (replace). A stale or unparseable lease is reclaimed under an exclusive `<lease>.lock` and removed only while it still holds exactly the record that was judged stale, so a fresh lease another racer published meanwhile is never deleted; crash-left `.lock`/`.tmp` files are swept after 30 s. A v1 lease (no `processStartedAtMs`) is a reuse only when the pid's process started after the lease was taken.
