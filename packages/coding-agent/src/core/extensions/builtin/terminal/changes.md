...
- A stale or unparseable lease is reclaimed by moving that exact file aside and checking it is the record that was inspected; a fresh lease another racer created meanwhile is put back, never deleted. A v1 lease (no `processStartedAtMs`) is a reuse only when the pid's process started after the lease was taken.
...
- A re-spawned watch's new `runtime` is written to the manifest right after the restore, so a second crash finds and stops that process instead of starting another copy. A restore that throws still decides (a `corrupt` digest) and keeps the lease bound for shutdown.
