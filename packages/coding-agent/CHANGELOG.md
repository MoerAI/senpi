...
- The handoff block's first moment is now the first message after the todo list exists, and the routing line no longer counts as a handoff, so weaker models report the plan to the user instead of treating the routing line as the report. ([#2121](https://github.com/code-yeongyu/senpi/issues/2121))
